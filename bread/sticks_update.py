...
from bread.constants import StickConstants, UserConstants
...
def update_stick_with_annotation(username, stick_id, score):
    ...
    new_polarity_score = (0.6 * bread['annotation']['modelPolarityScore'] + 0.4 * sum(
        [list(x.values())[0] for x in bread['annotation']['annotations']]) / 3)
